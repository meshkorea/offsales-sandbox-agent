...
async function ensureAppSession(c: any, requestedSessionId?: string | null): Promise<string> {
  assertAppWorkspace(c);
  const requested = typeof requestedSessionId === "string" && requestedSessionId.trim().length > 0
...
async function upsertStripeLookupEntries(
  c: any,
...
async function findOrganizationIdForStripeEvent(
  c: any,
  customerId: string | null,
  subscriptionId: string | null,
): Promise<string | null> {
...
async function listOrganizationMembers(c: any): Promise<FactoryOrganizationMember[]> {
  ...
  const rows = await c.db
    ...
    .orderBy(organizationMembers.role, organizationMembers.name)
    .all();
  return rows.map((row) => ({
    id: row.id,
    name: row.name,
...
async function listOrganizationSeatAssignments(c: any): Promise<string[]> {
  assertOrganizationWorkspace(c);
  const rows = await c.db
    .select({ email: seatAssignments.email })
    .from(seatAssignments)
    .orderBy(seatAssignments.email)
    .all();
...
export const workspaceAppActions = {
  ...
  async bootstrapAppGithubSession(
    c: any,
    input: { accessToken: string; sessionId?: string | null },
  ): Promise<{ sessionId: string; redirectTo: string }> {
    assertAppWorkspace(c);
  ...
  async createAppCheckoutSession(
    ...
    return {
      url: await appShell.stripe.createCheckoutSession({
        organizationId: input.organizationId,
        customerId,
  ...
  async finalizeAppCheckoutSession(
    c: any,
    input: { sessionId: string; organizationId: string; checkoutSessionId: string },
  ...
  async createAppBillingPortalSession(
    c: any,
  ...
  async handleAppStripeWebhook(c: any, input: { payload: string; signatureHeader: string | null }): Promise<{ ok: true }> {
    ...
    if (event.type === "invoice.paid" || event.type === "invoice.payment_failed") {
      ...
      const organizationId = await findOrganizationIdForStripeEvent(
        c,
  ...
  async handleAppGithubWebhook(c: any, input: { payload: string; signatureHeader: string | null; eventHeader: string | null }): Promise<{ ok: true }> {
    ...
    if (event === "installation_repositories") {
      console.log(`[github-webhook] ${event}.${body.action} for ${accountLogin}: +${body.repositories_added?.length ?? 0} -${body.repositories_removed?.length ?? 0}`);
      const workspace = await getOrCreateWorkspace(c, organizationId);
    ...
    if (event === "push" || event === "pull_request" || event === "pull_request_review" || event === "pull_request_review_comment" ||
        event === "check_run" || event === "check_suite" || event === "status" || event === "create" || event === "delete") {
  ...
  async syncOrganizationShellFromGithub(c: any, input: {
    userId: string;
    userName: string;
    userEmail: string;
